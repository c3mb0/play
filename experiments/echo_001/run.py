#!/usr/bin/env python3
"""Bounded custody and independent acceptance for the PTY echo-state witness."""
import json
import os
...
def main():
    run = ROOT / "receipts" / ("echo-" + time.strftime("%Y%m%dT%H%M%SZ", time.gmtime()))
    run.mkdir()
    ...
    subject = ROOT / "target/debug/hello_witness"
    try:
        manifest["subject_sha256_before"] = digest(subject)
        result = subprocess.run(["mix","run",str(ROOT / "experiments/echo_001/run.exs")],
        ...
        for case,expected in {"pairs":["pass"]*3}.items():
            ...
            assert report["definition"]["order"] == ["echo_on","echo_off"]
            ...
            for pair in report["pairs"]:
                ...
                for cell in pair["cells"]:
                    ...
                    assert bytes.fromhex(cell["requested_input_hex"]) == b"hello\n"
                    assert bytes.fromhex(cell["observation"]["streams_hex"]["input_written"]) == b"hello\n"
                    ...
                    if cell["condition"] == "echo_on":
                        assert raw in [b"input> hello\r\nreceived: hello\r\n", b"hello\r\ninput> received: hello\r\n"]
                    else:
                        assert cell["condition"] == "echo_off"
                        assert raw == b"input> received: hello\r\n"
