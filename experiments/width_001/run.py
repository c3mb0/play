#!/usr/bin/env python3
"""Bounded custody and independent acceptance for the initial-width ls witness."""
import json
import os
import re
...
def main():
    run = ROOT / "receipts" / ("width-" + time.strftime("%Y%m%dT%H%M%SZ", time.gmtime()))
    run.mkdir()
    ...
    try:
        ...
        result = subprocess.run(["mix","run",str(ROOT / "experiments/width_001/run.exs")],
        ...
        for case,expected in {"pairs":["pass"]*3}.items():
            ...
            assert report["definition"]["order"] == ["wide","narrow"]
            ...
            for pair in report["pairs"]:
                ...
                for cell in pair["cells"]:
                    ...
                    columns = observed[0]["configuration"]["dimensions"]["cols"]
                    if cell["condition"] == "wide":
                        assert columns == 80 and re.fullmatch(rb"alpha[ \t]+bravo[ \t]+charlie\r\n", raw)
                    else:
                        assert cell["condition"] == "narrow" and columns == 8
                        assert raw == b"alpha\r\nbravo\r\ncharlie\r\n"
                ...
                wide, narrow = [c["observation"]["header"]["spec"] for c in pair["cells"]]
                assert wide["attachment"] == narrow["attachment"] == "slave"
                expected = json.loads(json.dumps(wide))
                expected["terminal"]["dimensions"]["cols"] = 8
