...
fn terminal(fd: libc::c_int) -> Value {
    ...
    unsafe {
        let tty = if libc::isatty(fd) == 1 { json!({"value": true}) }
            else { json!({"value": false, "error": os_error()}) };
        let fg = libc::tcgetpgrp(fd);
        let foreground = if fg < 0 { json!({"error": os_error()}) } else { json!({"value": fg}) };
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let observation = unsafe {
        ...
        let session = if sid < 0 { json!({"error": os_error()}) } else { json!({"value": sid}) };
        let controlling = libc::open(c"/dev/tty".as_ptr(), libc::O_RDONLY | libc::O_NOCTTY);
        let controlling_tty = if controlling < 0 { json!({"accessible": false, "error": os_error()}) }
            else { libc::close(controlling); json!({"accessible": true}) };
    ...
    let mut snapshot = OpenOptions::new().write(true).create_new(true).open(&args[2])?;
